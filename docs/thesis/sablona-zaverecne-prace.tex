...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\noindent Tato práce se zabývá návrhem a realizací systému pro sdílení domácích nabíječek elektromobilů, který uživatelům umožňuje zpřístupnit svou nabíječku ostatním podobně jako model Airbnb. V Česku sice není infrastruktura nabíjecích stanic kriticky nedostatečná, ale komunitní projekt tohoto typu může zvýšit jejich dostupnost, zlevnit nabíjení a urychlit návratnost domácích fotovoltaických elektráren. Existuje komunita uživatelů, kteří si nabíječky půjčují neformálně, avšak doposud se domlouvají složitě přes chatovací platformy. Cílem projektu je vytvořit funkční prototyp platformy, která dokáže komunikovat s domácími nabíječkami přes protokol OCPP 1.6, zpracovat RFID autorizaci a umožnit automatické účtování plateb za nabíjení. Implementace zahrnuje databázový model v PostgreSQL, autentizaci pomocí JWT a základní bezpečnostní prvky. Řešení je založeno na dvou backendových službách – OCPP serveru v Node.js a API serveru ve FastAPI. Součástí práce je také webová aplikace s mapovým zobrazením nabíječek. Registrovaní uživatelé mohou spravovat vlastní nabíječky, prohlížet historii nabíjení a spravovat svůj účet. Výsledkem je funkční prototyp systému umožňující registraci uživatelů i nabíječek, zpracování RFID přístupů, výpočet cen nabíjení a zobrazení nabíjecích míst na mapě. Projekt zároveň vytváří prostor pro budoucí rozšíření o rezervace, platební brány a podporu nabíječek bez nativní podpory OCPP.\\
	
	\vspace{18pt}
	
	\noindent{\large{\bfseries{Klíčová slova}}}
	
	\noindent OCPP 1.6, nabíjecí stanice, elektromobilita, sdílení nabíječek, RFID autorizace, FastAPI, Node.js, WebSocket server, PostgreSQL, SvelteKit, mobilní aplikace, mapová vizualizace, \dots 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
	...
	\noindent This thesis focuses on the design and implementation of a system for sharing home electric vehicle chargers, allowing users to make their chargers available to others in a manner similar to the Airbnb model. Although the charging infrastructure in the Czech Republic is not critically insufficient, a community-driven project of this type can increase charger availability, reduce charging costs, and accelerate the return on investment for home photovoltaic systems. A community of users who share chargers already exists, but they currently rely on informal and inefficient coordination through chat platforms. The goal of the project is to create a functioning prototype of a platform capable of communicating with home chargers via the OCPP 1.6 protocol, processing RFID authorization, and automatically calculating charging payments. The implementation includes a PostgreSQL database model, JWT-based authentication, and essential security features. The solution is built on two backend services — an OCPP server developed in Node.js and an API server using FastAPI. The work also includes a web application featuring a map-based display of available chargers. Registered users can manage their chargers, view charging history, and maintain their user accounts. The result is a functional prototype enabling user and charger registration, RFID processing, charging price calculation, and map-based visualization of charging locations. The project additionally lays the groundwork for future extensions, including reservations, payment gateways, and support for chargers without native OCPP compatibility. %% přepiš!!
	
	\vspace{18pt}
	
	\noindent{\large{\bfseries{Keywords}}}
	
	\noindent OCPP 1.6, charging station, electromobility, charger sharing, RFID authorization, FastAPI, Node.js, WebSocket server, PostgreSQL, SvelteKit, mobile application, map visualization, \dots 
...
\subsection{Co je OCPP protokol}
