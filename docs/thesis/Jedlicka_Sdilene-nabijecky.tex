...
Pro ověření v reálném provozu byla využita domácí nabíjecí stanice Solax X3-HAC 11kW. Cílem bylo potvrdit, že systém dokáže komunikovat i s fyzickým zařízením a řídit tok energie.
...
\subsection{Zjištěná specifika a řešení problémů}
Při úvodních testech se objevil problém, kdy i přes korektní průběh komunikační sekvence a potvrzení \texttt{StartTransaction} nedošlo u fyzické stanice k sepnutí silových prvků (stykačů). Zařízení po přibližně 15 sekundách relaci ukončilo zprávou \texttt{StopTransaction}.

Analýzou bylo zjištěno, že firmware konkrétní testované stanice vyžaduje z bezpečnostních důvodů striktně šifrované spojení i v rámci lokální sítě. Problém byl vyřešen přechodem z nezabezpečeného protokolu \texttt{ws://} na zabezpečený \texttt{wss://} (WebSocket Secure) s využitím SSL/TLS certifikátu. Po této úpravě začala stanice na příkazy serveru reagovat správně a fyzické nabíjení bylo úspěšně zahájeno.
